...
async function AddParagraphBasedQuestion(req, res) {
    const { paragraph, questions } = req.body; // Expecting an array of questions
    ...
    try {
        ...
        console.error("Error processing AddParagraphBasedQuestion:", error.message);
        return res.status(500).json({ success: false, message: "Error processing request", details: error.message });
    }
}

module.exports = {
    AddParagraphBasedQuestion
};