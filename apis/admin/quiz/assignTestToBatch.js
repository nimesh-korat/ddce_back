...
async function assignTestBatch(req, res) {
  const { tbl_batch, tbl_phase, tbl_test, start_date, end_date, isFeatured } =
    req.body;
  ...
  try {
    ...
    const checkSql = `
            ...
            WHERE tbl_batch = ? AND tbl_phase = ? AND tbl_test = ?
        `;

    const [existingRecords] = await pool
      .promise()
      .query(checkSql, [tbl_batch, tbl_phase, tbl_test]);

    if (existingRecords.length > 0) {
      return res.status(409).json({
        success: false,
        message: "This test is already assigned to this batch and phase.",
      });
    }
    ...
    const insertSql = `
            INSERT INTO tbl_test_assigned (tbl_phase, tbl_batch, tbl_test, start_date, end_date, isFeatured)
            VALUES (?, ?, ?, ?, ?, ?)
        `;
