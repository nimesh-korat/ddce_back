...
async function editTestBatch(req, res) {
  ...
  const sql = `
        ...
        SET tbl_batch = ?, tbl_phase = ?, tbl_test = ?, start_date = ?, end_date = ?, isFeatured = ?
        WHERE id = ?
    `;
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      return res
        .status(404)
        .json({ success: false, message: "Test batch not found" });
    }

    return res
      .status(200)
      .json({ success: true, message: "Test batch updated successfully" });
  } catch (err) {
