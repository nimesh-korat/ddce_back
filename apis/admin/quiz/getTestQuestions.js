...
async function getTestQuestions(req, res) {
  ...
  if (!test_id) {
    return res
      .status(400)
      .json({ success: false, message: "Missing required test_id" });
  }

  try {
    // Fetch questions for the given test, including paragraph details if available
    const sql = `
            SELECT 
                q.id AS question_id,
                q.question_text,
            ...
            LEFT JOIN tbl_paragraph p ON q.tbl_paragraph = p.paragraph_id  -- Left join to get paragraph details
            WHERE tq.test_id = ?
        `;
    ...
    const signedQuestions = questions.map((question) => {
      ...
      return {
        ...
        paragraph_img: generateSignedImageUrl(question.paragraph_img), // Signed URL for paragraph image
      };
    });
