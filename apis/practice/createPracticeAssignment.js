...
async function createPracticeAssignment(req, res) {
  ...
  if (!practice_id || !tbl_batch)
    return res.status(400).json({
      success: false,
      message: "practice_id and tbl_batch are required",
    });

  try {
    // Verify practice exists and requester has rights
    const [practice] = await pool
      .promise()
      .query(
        "SELECT id, added_by FROM tbl_practice WHERE id = ? AND is_active = 1",
        [practice_id],
      );
    ...
    if (role === 2 && practice[0].added_by !== user_id)
      return res.status(403).json({
        success: false,
        message: "You can only assign your own practices",
      });
    ...
    await pool.promise().query(
      `INSERT INTO tbl_practice_assigned
         (practice_id, tbl_batch, tbl_phase, start_date, end_date, is_featured, assigned_by)
       VALUES (?, ?, ?, ?, ?, 1, ?)`,
      [
        practice_id,
        tbl_batch,
    ...
    return res.status(201).json({
      success: true,
      message: "Practice assigned to batch successfully",
    });
  } catch (err) {
