...
async function deletePracticeAssignment(req, res) {
  ...
  if (!id)
    return res
      .status(400)
      .json({ success: false, message: "Assignment ID required" });

  try {
    const [existing] = await pool
      .promise()
      .query("SELECT id, assigned_by FROM tbl_practice_assigned WHERE id = ?", [
        id,
      ]);
    ...
    await pool
      .promise()
      .query("DELETE FROM tbl_practice_assigned WHERE id = ?", [id]);
    return res
      .status(200)
