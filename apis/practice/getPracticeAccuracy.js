...
async function getPracticeAccuracy(req, res) {
  ...
  try {
    // ── EXAM data — only count rows where is_count = 0 ───────────
    const examSql = `
      SELECT
      ...
      LEFT JOIN tbl_student_answer sa
        ON tq.question_id = sa.question_id
        AND sa.student_id = ?
        AND sa.is_count   = 0
    ...
    // ── PRACTICE data — only count rows where is_count = 0 ───────
    const practiceSql = `
      SELECT
        sub.Id          AS subject_id,
        sub.Sub_Name    AS subject_name,
        ...
        0                                                               AS total_skipped
      FROM tbl_subject sub
      LEFT JOIN tbl_practice_answer pa
        ON pa.subject_id = sub.Id
        AND pa.student_id = ?
        AND pa.is_count   = 0
      GROUP BY sub.Id, sub.Sub_Name
      ...
      rows.forEach((r) => {
        if (!subjectMap[r.subject_id]) {
          subjectMap[r.subject_id] = {
            ...
            TotalQuestionsAsked: 0, TotalCorrect: 0,
            TotalIncorrect: 0, TotalSkipped: 0,
            TotalAttempted: 0, Accuracy: 0,
          };
        }
    ...
    OverallAnalytics.Accuracy = OverallAnalytics.TotalAttempted > 0
      ? Math.round((OverallAnalytics.TotalCorrect / OverallAnalytics.TotalAttempted) * 100 * 100) / 100
      : 0;

    return res.status(200).json({ success: true, OverallAnalytics, SubjectWiseAnalytics });
  } catch (err) {
