...
async function getWrongPracticeAnswers(req, res) {
  ...
  try {
    const subjectCondition = subject_id ? "AND pa2.subject_id = ?" : "";
    ...
    const sql = `
      ...
      JOIN tbl_questions q ON q.id = pa2.question_id
      JOIN tbl_practice_assigned pq_assign ON pq_assign.id = pa2.practice_assigned_id
      LEFT JOIN tbl_subject sub ON sub.Id = pa2.subject_id
      WHERE pa2.student_id = ?
        AND pa2.is_correct = '0'
        AND pq_assign.tbl_batch = ?
        AND (pq_assign.tbl_phase IS NULL OR pq_assign.tbl_phase = ?)
    ...
    const [results] = await pool.promise().query(sql, [
      student_id, batch_id, phase_id, ...subjectParam,
    ]);
    ...
    return res.status(200).json({ success: true, data: signed, total: signed.length });
  } catch (err) {
