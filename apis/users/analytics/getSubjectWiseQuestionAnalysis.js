...
async function GetSubjectWiseAnalysis(req, res) {
    try {
        ...
        // Execute the queries
        const [subjectResults] = await pool.promise().query(subjectSql, [studentId]);
        const [overallResults] = await pool.promise().query(overallSql, [studentId]);
