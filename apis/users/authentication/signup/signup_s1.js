...
async function SignupUser_s1(req, res) {
    ...
    try {
        ...
        `;
        const [existingUser] = await pool.promise().query(sqlCheck, [Email_Id, Phone_Number]);

        if (existingUser.length > 0) {
            return res.status(400).json({
                success: false,
                message: "Email or Phone Number already exists. Please use a different one.",
            });
        }
        ...
        try {
            ...
            const smsResult = await sendSMS(`Your OTP is: ${Phone_OTP}`, Phone_Number);
        ...
        const sql = `
            INSERT INTO users 
            (Name, Email_Id, Role, Phone_Number, Whatsapp_Number, Password, Phone_OTP, Phone_otp_expire_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ? )
        `;
        const values = [
            Name,
