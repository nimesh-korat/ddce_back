...
async function GetSyllabusWithPaper(req, res) {
  ...
  try {
    // Check if exam_type_id column exists on tbl_test_type
    let hasExamTypeCol = false;
    try {
      await pool
        .promise()
    ...
    const formattedResults = results.reduce((acc, row) => {
      ...
      const subjectWeightage =
        row.SubjectWeightage % 1 === 0
          ? parseInt(row.SubjectWeightage)
          : row.SubjectWeightage;
      const topicWeightage =
        row.TopicWeightage % 1 === 0
          ? parseInt(row.TopicWeightage)
          : row.TopicWeightage;
      ...
      if (paperIndex === -1) {
        ...
        const subjectIndex = acc[paperIndex].Subjects.findIndex(
        ...
        if (subjectIndex === -1) {
          ...
          const topicIndex = acc[paperIndex].Subjects[
            subjectIndex
          ].Topics.findIndex((t) => t.TopicId === row.TopicId);
          if (topicIndex === -1) {
            ...
            acc[paperIndex].Subjects[subjectIndex].Topics[
              topicIndex
            ].Subtopics.push({
              SubtopicId: row.SubtopicId,
              Subtopic: row.Subtopic,
