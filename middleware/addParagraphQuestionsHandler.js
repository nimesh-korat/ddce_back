...
} = require("../apis/admin/questions/AddParagraphQuestions");
  ...
  try {
    ...
    parsedQuestions.forEach((question, index) => {
      question.question_image =
        req.files["question_image"]?.[index]?.filename || null;
      question.option_a_image =
        req.files["option_a_image"]?.[index]?.filename || null;
      question.option_b_image =
        req.files["option_b_image"]?.[index]?.filename || null;
      question.option_c_image =
        req.files["option_c_image"]?.[index]?.filename || null;
      question.option_d_image =
        req.files["option_d_image"]?.[index]?.filename || null;
      question.answer_image =
        req.files["answer_image"]?.[index]?.filename || null;
    });
